...
# ─── CONFIG ────────────────────────────────────────────────────────────────────
BASE_DIR    = r"C:\Users\fotis\Downloads"
SPLITS      = ["Training_2", "Validation_2", "Test_2"]
NUM_POINTS  = 256
BATCH_SIZE  = 32
DEVICE      = torch.device("cuda" if torch.cuda.is_available() else "cpu")
MODEL_PATH  = os.path.join(BASE_DIR, "pointnet_dock_model.pt")
...
def main():
    ...
    # ─── CLASS WEIGHTS ─────────────────────────────────────────────────────────
    labels = [label for _, label in loaders["Training_2"].dataset.data]
    ...
    for epoch in range(1,101):
        model.train()
        for xb, yb in loaders["Training_2"]:
        ...
        preds, trues = [], []
        with torch.no_grad():
            for xb, yb in loaders["Validation_2"]:
                xb = xb.to(DEVICE)
